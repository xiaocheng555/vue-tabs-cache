...
function getData () {
  ...
  setTimeout(() => {
    loading.value = false
    cateList.value = Array.from(new Set(userData.map(user => user.userName))).map(item => ({ name: item }))
    getUserList()
  }, 1000)
}

function getUserList () {
  userList.value = userData.filter(user => user.userName === curCate.value)
}
